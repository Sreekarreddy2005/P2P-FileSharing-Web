document.addEventListener('DOMContentLoaded', function() {
   ...
   function uploadFiles(formData) {
       ...
       .then(data => {
           if (data.file_urls) {
               ...
               const qrCodes = data.qr_codes; // Get QR codes from response

               // Display links and QR codes
               fileLink.innerHTML =
                   'Files uploaded successfully!<br>' +
                   urls.map(url => `<a href="${url}" target="_blank">${url}</a>`).join('<br>');
               
               qrCodesDiv.innerHTML =
                   qrCodes.map(qrCode => `<img src="${qrCode}" alt="QR Code" style="width:100px; height:100px; margin-right:10px;">`).join('');
               
               qrCodesContainer.style.display = 'block'; // Show QR codes container
   ...
   dropZone.addEventListener('drop', (event) => {
       ...
       displayFileNames(); // Reuse existing function to show selected files.
   });
});
